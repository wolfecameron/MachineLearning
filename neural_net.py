	...
	def backward_prop(self, inputs, outputs, expected, alpha=.01):
		...
		for layer in reversed(range(self.layers)):
			if(layer == self.layers - 1):
				...
				print("WEIGHTS SHAPE: " + str(self.weights[layer - 1].shape))
				
				updates = np.vstack([np.ones((1,1)), self.values[layer - 2]]).dot(delta).T
				
				print("UPDATES: " + str(updates.shape))
				self.weights[layer - 1] += updates
				print("MADE IT THRU FIRST")
			else:
				error = delta.dot(self.weights[layer - 1].T) 
				delta = error*sig_deriv(self.values[layer - 1])
				self.weights[layer - 1] += self.values[layer - 2].dot(delta)				
